...
function pegFactory(id, indexOnPeg, pegNumber, type) {
  ...
  newdiv.style.height = 190; //changed
  game.appendChild(newdiv);
}
...
// MY CODE HERE
function makeMove() {
  let numDiskField = document.getElementById('numDisk');

  if (numDiskField.value.length == 0) {
  ...
  if (counter == 0) {
    ...
    if (numDiskField.value) {
      nDisks = numDiskField.value;
    }
...
// MY CODE HERE; RESET BUTTON
function reset() {
    document.getElementById('numDisk').value = "";
}
